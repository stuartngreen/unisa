...
{
    if (n == 1) //First element is already at the front.
        return;

    if (n < 1 || n > this->count) //Invalid input.
    {
        cout << "Invalid selection." << endl;
        return;
    }

    n--; //Get the index of the one to move.
    Type moveToFront;

    //Swap the element to move with the one before it until it reaches the front of the queue.
    for (int i = n; i > 0; i--)
    {
        moveToFront = list[i];
        list[i] = list[i - 1];
        list[i - 1] = moveToFront;
    }
}
